...
@Slf4j
@Component
public class PayrollCollector extends AbstractPagedCollector<JSONObject> {
    ...
    public PageResult<JSONObject> collectPage(CollectContext ctx, int pageNum, int pageSize) {
        String payMonth = ctx.getExtraParam("payMonth") != null ? ctx.getExtraParam("payMonth").toString() : "";

        JSONObject data = apiClient.getPayrollPage(
                ctx.getSourceProjectNum(), pageNum, pageSize,
                payMonth, ctx.getAccount(), ctx.getPassword()
        );
