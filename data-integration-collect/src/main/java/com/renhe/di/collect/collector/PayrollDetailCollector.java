...
@Slf4j
@Component
public class PayrollDetailCollector extends AbstractPagedCollector<JSONObject> {
    ...
    public PageResult<JSONObject> collectPage(CollectContext ctx, int pageNum, int pageSize) {
        String salaryId = ctx.getExtraParam("salaryId") != null ? ctx.getExtraParam("salaryId").toString() : "";
        if (salaryId.isEmpty()) {
            return PageResult.empty();
        }

        JSONObject data = apiClient.getPayrollDetailPage(
                salaryId, pageNum, pageSize,
